...
@Component
public class WarningUseCase implements UseCase<LogParams>{

    final LoggerRepository repository;

    @Autowired
    public WarningUseCase(LoggerRepositoryImpl repository){
        this.repository = repository;
    }

    @Override
    public void call(LogParams logParams) {
        repository.w(logParams.getUTCDate(), logParams.getTag(), logParams.getMethod(), logParams.getMessage());
    }
}
